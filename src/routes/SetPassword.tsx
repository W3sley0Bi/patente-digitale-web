...
export default function SetPassword() {
	...
	const hashErrorMessage = (() => {
		...
		if (hashError.code === "access_denied")
			return t("auth.errors.accessDenied");
		return t("auth.errors.generic", {
			description: hashError.description || hashError.code,
		});
	})();
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const wantsAutoscuola =
			(next?.includes("driving-school") ?? false) ||
			!!localStorage.getItem("domain_claim");
		if (wantsAutoscuola && user && role !== "autoscuola") {
			await supabase
				.from("profiles")
				.update({ role: "autoscuola", approved: false })
				.eq("id", user.id);
			await refresh();
		}
		...
		<div className="min-h-screen bg-bg text-ink">
			...
			<div className="flex flex-col items-center justify-center min-h-[calc(100vh-5rem)] px-4 pt-20 pb-8">
				<div className="w-full max-w-sm flex flex-col gap-6">
					...
					{hashErrorMessage && (
						<div
							className="flex flex-col gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-3"
							role="alert"
						>
