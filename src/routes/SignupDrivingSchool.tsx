...
import {
	ClaimSearch,
	type SchoolMatch,
} from "@/components/driving-school/ClaimSearch";
...
export default function SignupDrivingSchool() {
			...
			.then(
				(data: {
					...
					const match = data.features.find(
						(f) => f.properties._placeId === placeIdParam,
					);
					if (!match) {
						setStep("not-found");
						return;
	...
	const goToManualClaim = () => {
		localStorage.removeItem("domain_claim");
		if (selected)
			localStorage.setItem(MANUAL_SCHOOL_KEY, JSON.stringify(selected));
		setStep("manual-claim");
	};
						...
						<div className="flex flex-col gap-3">
							<p
								...
								dangerouslySetInnerHTML={{
									__html: t("school.claim.alreadyClaimed", {
										name: selected.name,
									}),
								}}
							/>
					...
					{step === "domain-email" && selected && (
						<>
							<p
								className="text-sm text-ink-muted"
								dangerouslySetInnerHTML={{
									__html: t("school.claim.foundSchool", {
										name: selected.name,
										domain,
									}),
								}}
								...
								<>
									{selected ? (
										<p
											className="text-sm text-ink-muted"
											dangerouslySetInnerHTML={{
												__html: t("school.claim.manualReview", {
													name: selected.name,
												}),
											}}
										/>
										...
										<div className="flex flex-col gap-2">
											<p className="text-sm text-ink-muted">
												{t("school.claim.manualReviewReturn")}
											</p>
											<p className="text-xs text-ink-faint">
												{t("school.claim.manualReviewReturnHint")}
											</p>
										</div>
									)}
									<ClaimForm
					...
					{step === "done" && (
						<div className="text-center flex flex-col gap-4">
							<p className="font-semibold text-lg">
								{t("school.claim.submitted")}
							</p>
							<p className="text-sm text-ink-muted">
								{t("school.claim.submittedDesc")}
							</p>
							<button
								type="button"
								onClick={handleDone}
