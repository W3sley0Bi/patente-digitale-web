...
export function ClaimForm({
	...
	const [manualAddress, setManualAddress] = useState(
		draft?.manualAddress ?? schoolData?.address ?? "",
	);
	const [manualCity, setManualCity] = useState(
		draft?.manualCity ?? schoolData?.city ?? "",
	);
	const [manualZip, setManualZip] = useState(
		draft?.manualZip ?? schoolData?.zip ?? "",
	);
	const [manualRegion, setManualRegion] = useState(
		draft?.manualRegion ?? schoolData?.region ?? "",
	);
	const [manualPhone, setManualPhone] = useState(
		draft?.manualPhone ?? schoolData?.phone ?? "",
	);
	const [manualWebsite, setManualWebsite] = useState(
		draft?.manualWebsite ?? schoolData?.website ?? "",
	);
	const [pinPosition, setPinPosition] = useState<[number, number] | null>(
		() => {
			if (draft?.pinLat != null && draft?.pinLng != null)
				return [draft.pinLat, draft.pinLng];
			if (schoolData?.lat && schoolData?.lng)
				return [schoolData.lat, schoolData.lng];
			return null;
		},
	);
	...
	const handleSubmitClaim = async (e: React.FormEvent) => {
		...
		if (!authUser) {
			setError(t("school.claimForm.notAuthenticated"));
			setLoading(false);
			return;
		}
		...
		return (
			<div className="flex flex-col gap-4">
				<p className="text-sm text-ink-muted">
					{t("school.claimForm.createAccount")}
				</p>
				<AuthForm
					mode="signup"
					role="autoscuola"
	...
	return (
		<form onSubmit={handleSubmitClaim} className="flex flex-col gap-4">
			...
			<label className="flex flex-col gap-1 text-sm">
				...
				<input
					...
					onChange={(e) => {
						...
						setFieldErrors((p) => ({ ...p, schoolName: undefined }));
					}}
					className="border rounded-lg px-3 py-2.5 text-sm bg-bg focus:outline-none focus:ring-2 focus:ring-ink/20 transition"
				/>
			...
			<Button type="submit" disabled={loading}>
				{loading
					? t("school.claimForm.submitting")
					: t("school.claimForm.submit")}
			</Button>
		</form>
	);
}
