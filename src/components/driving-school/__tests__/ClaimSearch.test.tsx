...
vi.stubGlobal(
	...
	vi.fn().mockResolvedValue({
		ok: true,
		json: () =>
			Promise.resolve({ type: "FeatureCollection", features: mockFeatures }),
	}),
);
...
describe("ClaimSearch", () => {
	it("renders a search input", () => {
		...
		expect(
			screen.getByPlaceholderText("school.search.placeholder"),
		).toBeInTheDocument();
	});

	it("shows results matching the query", async () => {
		render(<ClaimSearch onSelect={vi.fn()} />);
		fireEvent.change(screen.getByPlaceholderText("school.search.placeholder"), {
			target: { value: "roma" },
		});
		await waitFor(() =>
	...
	it("does not show results that don't match", async () => {
		...
		await waitFor(() =>
			expect(
				screen.queryByText("Autoscuola Napoli Sud"),
