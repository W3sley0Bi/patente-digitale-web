-- 026: claim_student_record must only claim ACTIVE unclaimed rows.
-- Unclaimed rows are only ever created active (add_student_manual); any other
-- status must not be claimable by token. Tightens the previous <> 'left' filter.
